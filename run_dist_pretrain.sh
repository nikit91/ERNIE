...
#run master node
./run-master-node.sh 4d $WORLD_N $TRAINING_DATA $OUT_DIR $PRETRAIN_FILE $EMBED_FILE $LIM_FLAG
#Ask for process id
...
#start normal nodes
./run-normal-nodes.sh 4d $MASTER_PROC_LOG_FILE $WORLD_N $TRAINING_DATA $OUT_DIR $PRETRAIN_FILE $EMBED_FILE $LIM_FLAG